...
function bindEvent() {
  ...
  $('input[name=ethAddress]').on('blur', function (e) {
    ...
    if(!isErr) err = 'Please input right eth address'
    showErr(me, err)
  })
      ...
      if (result.success) {
        ...
        if(result.data){
          if(result.data.kycStatus === 1){
            ...
            $('.btn-submit').attr('disabled', 'disabled')
            var txt = $('.page-title').text()
            var html = '<span style="color: #4393d8; display: block;"> The information you submitted has been uploaded successfully, waiting for the staff to review.(您提交的信息已上传成功，等待工作人员审核中) </span>'
            $('.page-title').html(txt + '' + html)
          } else if(result.data.kycStatus === 0){
            // 0 处理中 1成功 2身份证姓名不符 3 认证失败
            var txt = $('.page-title').text()
            var html = '<span style="color: #4393d8; display: block;"> Processing(审核中)... </span>'
            $('.page-title').html(txt + ' ' + html)
          } else if(result.data.kycStatus === 2){
            // 0 处理中 1成功 2身份证姓名不符 3 认证失败
            var txt = $('.page-title').text()
            var html = '<span style="color: #4393d8; display: block;"> The ID Card name fail to match your name(审核失败)... </span>'
            $('.page-title').html(txt + ' ' + html)
          } else if(result.data.kycStatus === 3){
            // 0 处理中 1成功 2身份证姓名不符 3 认证失败
            var txt = $('.page-title').text()
            var html = '<span style="color: #4393d8; display: block;"> Authentication fail(审核失败)... </span>'
            $('.page-title').html(txt + ' ' + html)
          }
        }
