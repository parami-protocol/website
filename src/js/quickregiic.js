function bindRegByPhone(){
	...
	$.ajax({
      ...
      if (!result.success) {
        ...
        window.location.href = IIC_QUICK_REGISTER_SUCCESS+'?flag=success&code='+result.data.result;
      }
		...
		if ($.trim(telInput.val())) {
          ...
          regSource:"IIC"
		    ...
		    if(result.success){
				...
				alert(result.message);
        if(result.msgCode == 10104 || result.msgCode == 10113){
          if (window.location.href === 'https://chain.pro/tpl/iic/reg.html?source=wechat') {
                window.location.href = IIC_LOGIN + '?source=wechat';
            } else {
                window.location.href = IIC_LOGIN;
            }
        }
			}
	   }
...
// $(function(){
//   var nc_token = ["FFFF00000000017B2097", (new Date()).getTime(), Math.random()].join(':');
